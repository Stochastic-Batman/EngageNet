...
\begin{document}
	...
	The Gumbel-max trick \cite{jang2017} is a technique for sampling from a categorical distribution by adding independent Gumbel-distributed noise to log-probabilities and taking the argmax. Critically, it enables backpropagation through discrete sampling steps by relaxing the argmax to a continuous softmax, acting as a differentiable approximation for optimising over discrete choices. We apply this principle at the level of permutation matrices: to enable stochastic exploration of orderings during training, the score matrix is perturbed with i.i.d.\ standard Gumbel noise and rescaled by temperature $\tau > 0$:
	$$\tilde{Z}_{ij} = \frac{Z_{ij} + G_{ij}}{\tau}, \qquad G_{ij} = -\ln(-\ln U_{ij}), \quad U_{ij} \overset{\text{i.i.d.}}{\sim} \mathrm{Uniform}(0,1).$$
	The formula $G_{ij} = -\ln(-\ln U_{ij})$ is the standard inverse-CDF transform for the Gumbel distribution \cite{gumbel1954}. The temperature $\tau$ is annealed from an initial value $\tau_0$ (e.g.\ $\tau_0 = 1.0$) to a small final value $\tau_\infty$ (e.g.\ $\tau_\infty = 0.05$) over the course of training, progressively concentrating probability mass towards a single hard permutation.
	
	\paragraph{Sinkhorn normalisation.}
	The \textbf{Birkhoff-von Neumann theorem} \cite{birkhoff1946} states that the set of doubly-stochastic matrices (square matrices with non-negative real entries in which every row and every column sums to exactly 1) is a convex polytope whose extreme points are precisely the permutation matrices; this polytope is known as the Birkhoff polytope. Any doubly-stochastic matrix can therefore be written as a convex combination of permutation matrices, making it a principled continuous relaxation of a discrete permutation. A \emph{hard permutation} is a matrix $\Pi \in \{0,1\}^{M \times M}$ that is a permutation matrix exactly - one entry equal to 1 per row and per column, all others 0 - representing a deterministic, non-differentiable assignment. A \emph{differentiable relaxation} replaces this with a doubly-stochastic matrix $P \in [0,1]^{M \times M}$ that lives in the interior of the Birkhoff polytope, where gradients are well-defined. Following \cite{mena2018}, we obtain such a $P$ by applying the Sinkhorn-Knopp algorithm to $\exp(\tilde{Z})$:
	$$P = \text{Sinkhorn}\bigl(\exp(\tilde{Z})\bigr).$$
	The Sinkhorn-Knopp algorithm alternately normalises the rows and columns of a positive matrix until convergence, and it can be shown that this iteration converges to the unique doubly-stochastic matrix in the same scaling class as $\exp(\tilde{Z})$. Because $\exp(\tilde{Z})$ is elementwise positive and the normalisation steps are differentiable, the gradient $\displaystyle \frac{\partial P}{\partial Z}$ exists and is well-conditioned throughout training, enabling end-to-end learning of the ordering via standard backpropagation.
