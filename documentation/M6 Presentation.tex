...
\begin{document}
	...
	\begin{frame}{What Changed, In Numbers}
		\begin{center}
			...
			\begin{tabular}{lrr}
				...
				\hline
				\textbf{Best validation CCC} & \textbf{0.0227} & \textbf{0.4228} \\
			\end{tabular}
		\end{center}
		\vspace{0.5em}
		\centering\small
		An $18.6\times$ improvement. Same data, same optimiser, same window length.
	\end{frame}

	\begin{frame}{\textsc{Part 3:} The Retrained Run}
		\begin{columns}[onlytextwidth]
			\begin{column}{0.46\textwidth}
				\begin{block}{Run summary}
					\small
					\begin{tabular}{lr}
						Wall time & 36 min 38 s \\
						XLA compile & 6 min 35 s \\
						Per epoch & $\sim$73 s \\
						Epochs run & 25 of 50 \\
			...
			\end{column}
			\begin{column}{0.50\textwidth}
				\begin{block}{Best epoch (15)}
					\small
					\begin{tabular}{lr}
						val CCC & \hl{0.4228} \\
						$\text{CDD}_G$ & $+0.0053$ \\
						$\text{CDD}_L$ (Chinese) & $-0.027$ \\
						$\text{CDD}_L$ (Japanese) & $+0.009$ \\
					\end{tabular}
					\vspace{0.3em}

					\footnotesize Our own success criterion was $\text{CDD} \le 0.05$.
				\end{block}
			\end{column}
		\end{columns}
		\vspace{0.4em}
		\centering\small
		Fairness is comfortably inside target. Accuracy is the part still moving.
	\end{frame}

	\begin{frame}{The Run Was Unstable - And Trained On The Wrong Corpus}
		\begin{columns}[onlytextwidth]
			\begin{column}{0.50\textwidth}
				\begin{alertblock}{Validation CCC bounced hard}
					\footnotesize
					$0.10 \to 0.25 \to 0.19 \to 0.09 \to 0.24 \to 0.24 \to 0.37 \to 0.38$
					$\to 0.25 \to 0.21 \to 0.08 \to 0.08 \to 0.34 \to 0.33 \to \mathbf{0.42} \to 0.29 \dots$
					\vspace{0.3em}

					\small Meanwhile training loss fell \emph{monotonically},
					$0.058 \to -0.503$. Smooth loss, thrashing metric.
				\end{alertblock}
			\end{column}
			\begin{column}{0.46\textwidth}
				\begin{alertblock}{And the corpus problem}
					\small
					The log shows only \texttt{NoXi+J/train} and \texttt{NoXi+J/val}.
					Languages: Japanese and Chinese only.
					\vspace{0.3em}

					\hl{NoXi was never loaded.} The model has never seen a European session.
				\end{alertblock}
			\end{column}
		\end{columns}
		\vspace{0.4em}
		\centering\small
		So $0.4228$ is an \emph{in-domain} number on Asian data. Cross-cultural transfer -
		the entire point of the TTA design - remains untested.
	\end{frame}
	...
	\begin{frame}{Status - Honest Version}
			...
			\begin{column}{0.48\textwidth}
				\begin{block}{Done \checkmark}
					...
					\begin{itemize}
						...
						\item Retrained end to end
						\item CCC $0.0227 \to 0.4228$
						\item CDD inside target on both axes
					\end{itemize}
				\end{block}
			\end{column}
			\begin{column}{0.48\textwidth}
				\begin{alertblock}{Caveats on that number}
					\small
					\begin{itemize}
						\item NoXi+J only - no European data
						\item Val split is 10 sessions; noisy
						\item Not comparable to the $0.400$ baseline, which is a mean over four \emph{test} sets
						\item Overfits after epoch 15
					\end{itemize}
				\end{alertblock}
			\end{column}
		\end{columns}
		...
		\begin{center}
			\begin{tikzpicture}
				...
				minimum width=11cm, minimum height=0.9cm, align=center,
				font=\small\bfseries, text=coolpurple] {
					The fixes clearly worked. Whether the model is competitive\\
					is a different question, and one we cannot answer yet.
				};
			\end{tikzpicture}
		\end{center}
	\end{frame}
	...
	\begin{frame}{Still Open}
			...
			\begin{column}{0.48\textwidth}
				\begin{alertblock}{Blocking}
					...
					\begin{itemize}
						...
						test sets, so we cap at 3/4 of the score
						\item TTA still never run end-to-end - and it is our headline contribution
					\end{itemize}
				\end{alertblock}
			\end{column}
			\begin{column}{0.48\textwidth}
				\begin{block}{Known gaps}
					\small
					\begin{itemize}
						\item No per-role or $\rho$ logging - cannot tell if the two heads collapsed
		...
		\begin{block}{Next, in order}
			\small
			1. Train on \textbf{NoXi} \quad
			2. Log per-role CCC and $\rho$ \quad
			3. Stabilise: $\eta = 2\times10^{-4}$, patience 5 \quad
			4. \textbf{Run TTA NoXi $\to$ NoXi+J} \quad
			5. Ablate the four loss terms
		\end{block}
	\end{frame}
	...
	\begin{frame}{What I Would Like From You}
		\begin{block}{Questions I cannot answer alone}
			...
			\begin{itemize}
				...
				models per corpus? The challenge requires us to declare which. Now that a run costs
				about an hour we can simply test both, but I would like agreement on the criterion first.
				\item \textbf{Val split size} - 10 sessions gives a CCC estimate that swings by
				$0.3$ between epochs. Do we accept that noise, or move to cross-validation?
			\end{itemize}
		\end{block}
		\vspace{0.6em}
		\begin{center}
