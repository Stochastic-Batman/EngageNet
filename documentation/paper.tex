...
\begin{document}
	...
	using $\operatorname{Cov}(y^{e}, y^{n}) = r\sigma^2$. The two participants agree only partly, so $r$ is well below $1$ and the averaged curve is markedly flatter than either original: at $r = 0.2$ it keeps $60\%$ of the variance. Since CCC is normalised by the variance of the target (Section 4.1), what is flattened away here cannot be recovered later.
