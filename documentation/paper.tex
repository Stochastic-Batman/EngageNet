...
\begin{document}
	...
	Since CCC is normalised by the variance of the target (Section 4.1), what is flattened away here cannot be recovered later.
	...
	One such head is instantiated per role, so a single forward pass yields $\bigl(\alpha_t^{r}, \beta_t^{r}\bigr)$ for each $r \in \mathcal{R}$. The two heads read the same fused representation and differ only in their read-out weights.
	...
	Training is stochastic: the initial weights, the order of training sessions and the Gumbel noise all depend on a random seed. Two runs that differ only in the seed produce different models and different scores, and a comparison between two configurations is only meaningful if their difference exceeds this spread. Every configuration below was therefore trained with three seeds ($95$, $1$, $2$), and we report the mean and the sample standard deviation.
	...
	\begin{tabular}{llccc c}
		...
		\midrule
		NoXi & $10$ s & $0.6987$ & $0.7111$ & $0.7170$ & $0.709 \pm 0.009$ \\
		NoXi & $20$ s & $0.7252$ & $0.6947$ & $0.7228$ & $0.714 \pm 0.017$ \\
		NoXi+J & $10$ s & $0.5298$ & $0.5481$ & $0.5082$ & $0.529 \pm 0.020$ \\
		NoXi+J & $20$ s & $0.5005$ & $0.6044$ & $0.5759$ & $0.560 \pm 0.054$ \\
		\bottomrule
	\end{tabular}
	\end{center}

	The seed-to-seed standard deviation lies between $0.01$ and $0.05$, so single-run differences of that size carry no information. The $20$ s windows ($L = 500$, hop $250$) were tested because after the S4D-Real initialisation the SSM can retain information for up to $\sim\!10^3$ steps, while a $10$ s window resets the state every $250$ steps. On NoXi the longer window changes nothing; on NoXi+J it raises the mean by $0.03$, but with a standard deviation of $0.054$ the difference is not established.

	\subsection{Ablation of the Training Objective}
	Each auxiliary term of the objective of Section \ref{sec:objective} was removed in turn, keeping everything else fixed ($10$ s windows, three seeds each). The base likelihood itself is kept in every variant: without it the concentration $\kappa = \alpha + \beta$ is not constrained by any term. Differences to the full objective are tested with a two-sided Welch $t$-test ($n = 3$ per group).
	...
	\begin{tabular}{lcccccc}
		...
		\textbf{Variant} & \textbf{Mean $\pm$ sd} & $\Delta$ & $p$ & \textbf{Mean $\pm$ sd} & $\Delta$ & $p$ \\
		\midrule
		Full & $0.709 \pm 0.009$ & -- & -- & $0.529 \pm 0.020$ & -- & -- \\
		No ordering ($\lambda_c = 0$) & $0.689 \pm 0.019$ & $-0.020$ & $0.20$ & $0.396 \pm 0.059$ & $-0.132$ & $0.048$ \\
		No per-modality ($\lambda_u = 0$) & $0.674 \pm 0.040$ & $-0.035$ & $0.26$ & $0.498 \pm 0.055$ & $-0.031$ & $0.44$ \\
		No reweighting ($\beta_w = 0$) & $0.672 \pm 0.037$ & $-0.037$ & $0.22$ & $0.495 \pm 0.024$ & $-0.034$ & $0.13$ \\
		No fairness ($\lambda_f = 0$) & $0.710 \pm 0.009$ & $+0.001$ & $0.91$ & $0.510 \pm 0.071$ & $-0.019$ & $0.69$ \\
		\bottomrule
	\end{tabular}
	\end{center}

	\textbf{Ordering term.} Removing $\lambda_c(1 - \widehat{\text{CCC}})$ causes the largest loss, and the only one that is individually significant: on NoXi+J the mean falls from $0.529$ to $0.396$. It is also the only term that addresses the evaluation metric directly; the likelihood grades each frame separately and has no term that rewards the correct relative order of frames.

	\textbf{Per-modality supervision and likelihood reweighting.} Removing either lowers the mean by $0.03$ to $0.04$ on both corpora. With three seeds per group none of these four differences is significant on its own, but all four point in the same direction. We read this as consistent evidence of a small benefit, not as a demonstrated one.

	\textbf{Stability.} The full objective also gives the most reproducible results: in $7$ of the $8$ ablated configurations the seed-to-seed standard deviation is larger than that of the full objective (NoXi: $0.009$ against up to $0.040$; NoXi+J: $0.020$ against up to $0.071$). The auxiliary terms therefore reduce the variance of training as well as, on average, raising its outcome.

	\textbf{Fairness term.} Removing the fairness penalty costs no accuracy. It also has no measurable effect on the quantity it targets:

	\begin{center}
	\small
	\begin{tabular}{lcccc}
		\toprule
		& \multicolumn{2}{c}{\textbf{NoXi}, $\text{CDD}_G$} & \multicolumn{2}{c}{\textbf{NoXi+J}, $\text{CDD}_G$} \\
		& \textbf{per seed} & \textbf{mean} $|\cdot|$ & \textbf{per seed} & \textbf{mean} $|\cdot|$ \\
		\midrule
		$\lambda_f = 0.05$ & $+0.0056,\ -0.0122,\ +0.0029$ & $0.0069$ & $-0.0003,\ -0.0387,\ -0.0357$ & $0.0249$ \\
		$\lambda_f = 0$ & $+0.0015,\ -0.0152,\ -0.0188$ & $0.0118$ & $-0.0288,\ -0.0233,\ -0.0253$ & $0.0258$ \\
		\bottomrule
	\end{tabular}
	\end{center}

	The gender disparity is small in both conditions and does not decrease when the penalty is active. Two properties of the current setting plausibly explain this. The weight $\lambda_f = 0.05$ is small next to the other terms, and a batch holds only $8$ windows, so after splitting its frames into $n_b = 10$ quantile bins many bins contain only one gender and contribute nothing to $\mathcal{L}_{\text{fair}}$ (Section \ref{sec:objective}). A larger weight and batches assembled to contain both genders are the natural next steps; as the objective stands, the fairness term should be reported as inactive rather than as effective.

	\subsection{Cross-Corpus Transfer: a Shift in Label Scale}
	A model trained on one corpus and evaluated on the other scores close to zero. The decomposition $\text{CCC} = \rho\,C_b$ of Section \ref{sec:ccc} shows where the loss arises:
	...
	\begin{tabular}{llccccc}
		...
		\midrule
		NoXi & NoXi & $0.699$ & $0.703$ & $0.995$ & $0.584$ & $0.570$ \\
		NoXi & NoXi+J & $0.092$ & $0.422$ & $0.219$ & $0.577$ & $0.834$ \\
		NoXi+J & NoXi+J & $0.530$ & $0.530$ & $0.999$ & $0.838$ & $0.834$ \\
	...
	\begin{tabular}{lcccc}
		...
		$(0.3,\ 0.7,\ 10^{-4})$ & $\approx 2\%^{\ddagger}$ & $0.0921$ & -- & $0.5339$ \\
		$(0.5,\ 0.5,\ 10^{-4})$ & $16.7\%$ & $0.0927$ & $17.2\%$ & $0.5332$ \\
		$(0.5,\ 0.5,\ 10^{-3})$ & $14.7\%$ & $0.0953$ & $18.4\%$ & $0.4178$ \\
		$(0.3,\ 0.7,\ 10^{-3})$ & $4.5\%$ & $0.0897$ & $5.5\%$ & $0.4796$ \\
		\bottomrule
	\end{tabular}
	\end{center}
	{\footnotesize $^{\ddagger}$ Estimated from the first three sessions; the fraction was not logged for the full run.}

	No setting changes the cross-corpus score by more than $0.004$. In-corpus, the smaller learning rate is neutral within seed noise ($+0.004$) and the larger one is harmful ($-0.05$ to $-0.11$). At the design quantiles $(0.3, 0.7)$ the filter selects only $2$ to $6\%$ of the windows, far below the $9\%$ that two independent conditions of $30\%$ each would give; presumably the fused and the unimodal uncertainties tend to rise and fall together, which makes the joint condition rare, although we have not measured this correlation. Taken together with the previous subsection, the result is a clear negative one: in its current form the adaptation neither closes the cross-corpus gap, which lies in the label scale, nor improves in-corpus predictions. All other results in this document are obtained without it.

	\subsection{Ensembling}
	For each corpus we average, frame by frame, the predictions of six models: the three seeds of the seed-variability experiment above, at both window lengths. Every member is trained on the training split only, and all six are used; no member is chosen by its validation score, since selecting the best single run would mostly select the luckiest seed.

	\textbf{Label-free rescaling.} Averaging cancels the members' independent errors but also flattens the prediction: on NoXi+J the members' predictions have a standard deviation of about $0.10$, the ensemble's only $0.082$, against $0.102$ for the annotations. This lowers $C_b$. We therefore stretch the ensemble back to the average spread and level of its members,
	$$\hat{y}'_t = \bar{\mu}_{\text{mem}} + \bigl(\hat{y}_t - \mu_{\text{ens}}\bigr)\,\frac{\bar{\sigma}_{\text{mem}}}{\sigma_{\text{ens}}},$$
	where $\mu_{\text{ens}}$ and $\sigma_{\text{ens}}$ are the mean and standard deviation of the ensemble over all frames of the split, and $\bar{\mu}_{\text{mem}}$, $\bar{\sigma}_{\text{mem}}$ are the averages of the members' own means and standard deviations over the same frames; the result is clipped to $[0,1]$. Only predictions enter this transformation, never annotations, so it applies unchanged to the test split. Being affine, it leaves $\rho$ untouched and can only move $C_b$.
	...
	\begin{tabular}{lcccccc}
		...
		\midrule
		Single model, $10$ s (mean of 3 seeds) & $0.709$ & -- & -- & $0.529$ & -- & -- \\
		Ensemble, 3 seeds, $10$ s & $0.728$ & $0.734$ & $0.992$ & $0.615$ & $0.641$ & $0.959$ \\
		Ensemble, 3 seeds, $20$ s & $0.736$ & $0.743$ & $0.990$ & $0.621$ & $0.647$ & $0.961$ \\
		Ensemble, all 6 & $0.740$ & $0.746$ & $0.991$ & $0.643$ & $0.677$ & $0.951$ \\
		Ensemble, all 6, rescaled & $\mathbf{0.742}$ & $0.746$ & $0.995$ & $\mathbf{0.661}$ & $0.677$ & $0.977$ \\
		\bottomrule
	\end{tabular}
	\end{center}

	Ensembling raises the correlation itself, from $0.703$ to $0.746$ on NoXi and from $0.530$ to $0.677$ on NoXi+J (single models of seed $95$), which is precisely the factor that the correlation ceiling of Section \ref{sec:ccc} identifies as the only remaining margin. The rescaling then recovers most of the accuracy lost to averaging. Two caveats apply: the choice of six members over three and the decision to rescale were made on the validation split, so the final numbers are slightly optimistic; and an ensemble is a statement about the best system we can build, not about the method. We therefore report the single-model mean over seeds as the estimate of the method and the rescaled six-model ensemble as the submitted system.
	...
	\begin{tabular}{llr}
		...
		-- & Total parameters (core subset) & $\approx 2.06$M \\
		...
		$n_{\text{iter}}$ & Sinkhorn normalisation iterations & $10$ \\
		...
		-- & Seeds for reported results & $95,\ 1,\ 2$ \\
