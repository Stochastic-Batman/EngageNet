...
\begin{document}
			...
			\begin{column}{0.48\textwidth}
				\begin{block}{Scorecard}
					...
					\begin{tabular}{lccc}
						...
						Fast training & $\times$ & \checkmark & \checkmark \\
					\end{tabular}
				\end{block}
			\end{column}
		\end{columns}
	...
	\begin{frame}{Responsible LLM Use}
		\textit{Claude Sonnet 4.6} was used to code the following files (I was too lazy to write them myself):
		\begin{itemize}
			\item All 3 test files: \texttt{tests/test\_frontend.py}, \texttt{tests/test\_model.py} and \texttt{tests/test\_inter\_modal.py}
			\item Forgot to include CCC, and I wanted to reach a milestone before \texttt{git push}: \texttt{src/evaluate.py}
			\item I did not figure out a neat way to do the stuff: \texttt{surgical\_mask} method in \texttt{src/tta.py} 
		\end{itemize}
	\end{frame}
